...
# Load Data ----
  ...
  mydata.12 <- read_dta("Data/anes_timeseries_2012.dta")
  mydata.cdf <- read_dta("Data/ANES-CDF.dta")
...
# 2016 ----
# Select variables
  #justincase <- mydata.16
  #mydata.16 <- justincase
  mydata.16 <- mydata.16 %>% 
...
# Select variables
  mydata.12 <- mydata.12 %>% 
    select(libcpre_self, inc_incgroup_pre, dem_edugroup_x, dem_racecps_white,
           ...
           gender_respondent_x, pid_x, rvote2012_x, presvote2012_x, 
...
# Select variables
  mydata.cdf <- mydata.cdf %>%
    select(VCF0004, VCF0605, VCF0604, VCF0624, VCF0703, 
           ...
           VCF0803, VCF0114, VCF0310, VCF9026)
...
# Rename variables
  ...
  mydata.cdf <- mydata.cdf %>% 
    ...
    mutate(voteprimary = ifelse(voteprimary == 1 | voteprimary == 2 | voteprimary == 3 | voteprimary == 5, 1,
                         ifelse(voteprimary == 7, 0, NA))) %>%
